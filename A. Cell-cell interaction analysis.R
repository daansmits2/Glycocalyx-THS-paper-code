...
rm(file)
file <- separate(file1, (ncol(file1)), into=c("ImageNR", "Acquisitiontype", "ROINR", "CellNR", 
                                              "Lineprofiletype", "Section", "Channel", "Interactiontype"), sep="_", remove=FALSE)
...
names(file)[names(file) =="Channel"] <- "Linethickness"

file$Linethickness <- as.numeric(file$Linethickness)

file$Acquisitiontype <- as.numeric(file$Acquisitiontype)

names(file)[names(file) =="Acquisitiontype"] <- "Exposure"


file <- file %>%  mutate(expnr = match(Subfolder, unique(Subfolder)))

file <- file %>% mutate(number= group_indices(.,Exposure, Linethickness, Subfolder, expnr, ImageNR , ROINR, CellNR))

file <- file %>% mutate(analysisnumber= group_indices(.,expnr, Lineprofiletype, CellNR, ImageNR, Subfolder, Section, ROINR, Exposure))
...
file_averaged <- file_averaged %>% 
  group_by(Exposure, Linethickness, Lineprofiletype, name, Subfolder, ImageNR, ROINR, 
           CellNR, number, analysisnumber, Section, X_group) %>% 
...
file <- left_join(file, transition_lengths, by = "number")
...
file_summarized <- file %>%
  group_by(Subfolder, expnr , number, ID, ImageNR , CellNR, ROINR, Lineprofiletype, Exposure , Linethickness, Section, Transitionzone_length) %>%
...
k <- ggplot(data = subset(setforplot), aes(x = X_micron_normalized, y = Value_relative)) +
  ...
  geom_line(data=subset(setforplot), aes(y=Value_relative, group=number, color=Section), size=0.5, alpha=0.5)+
  stat_smooth(method = "loess", span = best_span, se = TRUE, color = "black", alpha=0.8, size=0.5) +
...
violin <- custom_violinplot_with_statistics(setforplot, group_vars=NULL,x_var="Section",
                                            y_var="Value_bgcor", fill_var="Section",jitter=F, plot_mean=FALSE)
stats <- violin$statistical_test
print(stats)
violin <- violin$plot + ylab("MFI") + theme(legend.position = "none")+
...
violin + geom_point(aes(color=expnr)) + geom_line(aes(group=number, color=expnr))

median(setforplot$Value_bgcor[setforplot$Section=="Single-cell segment"])
...
selection <- dotplot.subset %>%  ungroup() %>%  dplyr::select(Transitionzone_length, Value_relative)
print(selection,n=100)
...
boxplot_stats <- data_optimallinethickness %>%
  ungroup() %>% 
  group_by(expnr, Section) %>%
...
stats <- data_optimallinethickness %>% filter(Section!="Background") %>%  ungroup() %>%  dplyr::select(Value_bgcor, expnr, Section)
print(stats, n=150)


## plot violin with 2-fold cell-cell contact -----

fold2 <- boxplot_stats$median[boxplot_stats$Section=="Single-cell segment"] *2
print(fold2)

violin + geom_hline(yintercept=fold2, linetype="dotted")