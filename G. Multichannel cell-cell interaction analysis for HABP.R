...
setforplot <- setforplot %>%  filter(Channel=="HABP")
...
k <- k + 
  stat_smooth(method = "loess", span = best_span, se = TRUE, color = "black", alpha=0.3, size=0.5, aes(group=Channel))+ scale_y_continuous(breaks=c(0,1,2,3)) + ylim(0,3)
...
ggplot(setforplot, aes(x=Section, y=Value_bgcor))+
  geom_violin(scale="width")+
  geom_boxplot(width=0.5, outlier.shape=NA)+
  geom_point()
...
violin <- custom_violinplot_with_statistics(subset(setforplot, Section=="Cell-cell-contact"), group_vars=NULL,x_var="Section",
                                            y_var="Value_bgcor",jitter=F, plot_mean=FALSE)
...
violin.enrichment <- violin + geom_point()   + geom_hline(yintercept=2, linetype=2) + facet_wrap(Channel~Section)+ 
  scale_y_continuous(breaks=c(0,2,4,6,8,10, 12, 14), limits=(c(0,14))) + ylab("Enrichment factor") + xlab("")
...
boxplot_stats <- setforplot %>%
  ungroup() %>%
  group_by(expnr, Channel, Section, Permeabilization_status) %>%
  summarise(
    n = n(),
    mean = mean(Value_bgcor, na.rm = TRUE),
    sd = sd(Value_bgcor, na.rm = TRUE),
...
superviolin <- setforplot %>% ungroup() %>%  filter(Channel=="HABP") %>%  dplyr::select(Value_bgcor, expnr, Section) %>%  filter(Section!="Background")  
